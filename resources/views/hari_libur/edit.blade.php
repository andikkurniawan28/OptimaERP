            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <form action="{{ route('hari_libur.update', $hari_libur->id) }}" method="POST">
                                @csrf
                                @method('PUT')

                                <div class="form-group">
                                    <label for="tanggal">{{ ucwords(str_replace('_', ' ', 'tanggal')) }}</label>
                                    <input type="date" name="tanggal" id="tanggal" class="form-control @error('tanggal') is-invalid @enderror" value="{{ old('tanggal', $hari_libur->tanggal) }}" required autofocus>
                                    @error('tanggal')
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name="keterangan" id="keterangan" class="form-control @error('keterangan') is-invalid @enderror" value="{{ old('keterangan', $hari_libur->keterangan) }}" required>
                                    @error('keterangan')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
