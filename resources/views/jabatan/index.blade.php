            ...
            <div class="page-inner py-5">
                <div class="d-flex align-items-left align-items-md-center flex-column flex-md-row">
                    ...
                    <div class="ml-md-auto py-2 py-md-0">
                        <a href="{{ route('jabatan.create') }}" class="btn btn-secondary btn-round">Tambah @yield('title')</a>
                    </div>
                </div>
            </div>
        </div>
                                ...
                                <table id="basic-datatables" class="table table-bordered">
                                    ...
                                    <tbody>
                                        @forelse ($jabatans as $jabatan)
                                            <tr>
                                                <td>{{ $jabatan->id }}</td>
                                                <td>{{ $jabatan->seksi->nama }}</td>
                                                <td>{{ $jabatan->nama }}</td>
                                                <td>
                                                    <a href="{{ route('jabatan.edit', $jabatan->id) }}" class="btn btn-warning btn-sm">Edit</a>
                                                    <form action="{{ route('jabatan.destroy', $jabatan->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Yakin ingin menghapus?')">
                                                        @csrf
                                                        @method('DELETE')
